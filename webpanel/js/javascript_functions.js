...
var deleteMessageText = "Are you sure you want to delete this item from the database?";
...
/* Ajax Deleting */
function sndMultipleDeleteReq(action) {
...
function deleteWithoutConfirm(ids, table, pk){

		ajax_deleteRows(ids, table, pk);
...
function ajax_deleteRows(ids, table, pk){
	var markki = "?";
	if (ajax_file.search("[?]") > 0) { markki = "&"; }
	//var req = ajax_file + markki + 'ajaxAction=delete&ids=' + ids + '&table=' + table + '&pk=' + pk; <--hides grid rows
	var req = ajax_file + markki + 'ajaxAction=delete&id=' + ids + '&table=' + table + '&pk=' + pk;
	sndMultipleDeleteReq(req);
}
